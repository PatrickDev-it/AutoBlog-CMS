...
const DEVELOPMENT_CRON_SECRET = 'development-only-cron-secret';
...
const environmentSchema = z.object({
	...
	CRON_SECRET: z.string().min(24).default(DEVELOPMENT_CRON_SECRET),
	...
	AUTH_SIGN_IN_RATE_LIMIT: z.coerce.number().int().min(3).max(100).default(8),
});
...
export function assertRuntimeConfiguration(environment = getEnvironment()): void {
	...
	if (environment.NODE_ENV === 'production' && environment.CRON_SECRET === DEVELOPMENT_CRON_SECRET) {
