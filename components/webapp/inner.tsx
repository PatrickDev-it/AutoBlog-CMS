...
export default ({ image, title, date, kind, description }: Omit<Post, 'image'> & { image: { public_id?: string; src?: string } }) => {
	...
	return (
		<div className="relative flex flex-col w-full gap-[--p] !text-black">
			<div className="flex flex-col gap-1 !text-black animate-[fade-in_0.5s_ease-in-out_forwards] delay-75">
				{kind && <label className="text-[.7rem] font-semibold text-gray-500 tracking-wide uppercase">{kind}</label>}
